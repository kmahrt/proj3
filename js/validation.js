...
/*
I used the template given and filled in all the TODOs
*/



let phoneRegex = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/;  //TODO go to regexer.com or reg101.com and find a suitable phone number regex and place here
...
function checkRequired(id, message) {
  ...
  //if any of the elements is 'checked', return true.
    let title = el.name;
    let titleAll = document.querySelectorAll(`input[name="${title}"]`);
    for (let one of titleAll) {
        if (one.checked) {
            valid = true;
            break;
        }
